...
using System.Xml;
//teacher log in (takes name instead of ID)
        ...
        private void button_login_Click(object sender, EventArgs e)
        {
            ...
            foreach (User user in registeredUsers)
            {
                if (user.Username == textBox_usernameLogin.Text && user.Password == textBox_passwordLogin.Text)
                {
                    ...
                    else if (user.Status.ToLower() == "teacher")
                    {
                    ...
                    else if (user.Status.ToLower() == "hc")
                    {
                        userData = $"{user.Name} {user.Lastname}\n{user.Status} - {user.Course}";
                        AdminForm adminForm = new AdminForm(user);
                        adminForm.Dock = DockStyle.Fill;
                        panelConteiner.Controls.Add(adminForm);
                        panelConteiner.Show();
                        panelConteiner.Controls["AdminForm"].BringToFront();
                    }
        ...
        private void button_register_Click(object sender, EventArgs e)
        {
            ...
            if (success)
            {
                ...
                if (newStatus.InnerText == "Student")
                {
                    ...
                    try
                    {
                        List<XmlNode> neededCourses = new List<XmlNode>();
                        ...
                        XmlNode targetCourse = neededCourses[0];
                        int currentCount = neededCourses[0].SelectSingleNode("students").ChildNodes.Count;
                        foreach (XmlNode course in neededCourses)
                        {
                            if (currentCount > course.SelectSingleNode("students").ChildNodes.Count)
                            {
                                targetCourse = course;
                            }
                        }
                        ...
                        newUserId.InnerText = (lastId + 1).ToString();
                        targetCourse.SelectSingleNode("students").AppendChild(newUserId);
